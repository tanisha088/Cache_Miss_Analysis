...
class Analysis extends LoopNestBaseListener {
    ...
    HashMap<String,Long> arraysizes;
    HashMap<String, String> upperbounds;
    String looplevelorder="";
    int cachePower, blockPower, wordPower ,arrayPower,N;
	...
	arraysizes =new HashMap<String,Long>(); 
	...
	looplevelorder="";
     ...
     {	
	   ...
	   arraysizes.put(variable_name, size);
        }
     ...
     @Override public void enterArrayAccess(LoopNestParser.ArrayAccessContext ctx) { 
		...
		Here, the variables which are used as array indices, are stored as a string in arrayindices HashMap 
		   EXAMPLE : for A[i][k] , "A" and "ik" would be stored as key and value respectively, in arrayindices 
 	     */
	 ...
	 */
	 ...
	 //we are dealing in terms of powers of 2, we can find arrayPower from array size as given below: 
	 arrayPower = (int)((Math.log(N)*dimension)/Math.log(2));       //It is assumed that size of each dimension of array is N
	 Long misses=1L;
	 ...
	 if(dimension==1)					       
	 {
		...
		*/
	     ...
	     char row=indices.charAt(1); 					
	     char column =indices.charAt(0);				
	   ...
	   */
	   ...
	   if(Math.pow(2,cachePower) >= Math.pow(2,arrayPower)) 
	   {
		misses = arraysizes.get(array)/(long)Math.pow(2,blockPower);
	   }    
	   else
	    switch(cacheType)
	    {
		      ...
		      for(int i; i<N; i++)
				...
				for(int k; k<N; k++)
				     sum = A[j][i] + B[i][j];
                      ...
                      here, value k doesn't affect the array indices. Thus, inne-most loop will not cause any cache miss.
		      B is accessed row-wise, so there would be one miss per block ,i.e, cache miss= (N*N)/blockSize.
		      A is accessed column-wise, so cache miss = (N*N)    
		*/
	   int i = columnvariablelevel;
	   long misses =1L;

		/* ASSUMPTION: cache is large enough to hold a row of array without any replacement. 
		   PATTERN NOTICED: 
	...
	@Override public void exitMethodBody(LoopNestParser.MethodBodyContext ctx) { 
		...
		N=Integer.parseInt(variables.get("N"));
